...
public class ParametersBenchmarks
{
    private Microsoft.AspNetCore.Http.Features.FeatureCollection f = new();
    private NZ.Orz.Features.FeatureCollection f2 = new();

    public ParametersBenchmarks()
    {
        var ts = AppDomain.CurrentDomain.GetAssemblies().SelectMany(i => i.GetExportedTypes()).Where(i => !i.IsGenericType && i.IsClass).ToList();
        foreach (var item in ts)
        {
            f.GetType().GetMembers().First(i => i.Name.StartsWith("Set")).Cast<MethodInfo>().MakeGenericMethod(item).Invoke(f, new object[] { null });
            f2.GetType().GetMembers().First(i => i.Name.StartsWith("SetFeature")).Cast<MethodInfo>().MakeGenericMethod(item).Invoke(f2, new object[] { null });
        }
        f.Set(this);
        f2.SetFeature(this);
    }
    ...
    public ParametersBenchmarks FGet()
    {
        return f.Get<ParametersBenchmarks>();
    }

    [Benchmark, BenchmarkCategory("Get")]
    public ParametersBenchmarks F2Get()
    {
        return f2.GetFeature<ParametersBenchmarks>();
    }
}
