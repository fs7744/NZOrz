...
namespace NZ.Orz.Http.Http2.FlowControl;
...
public sealed class InputFlowControl
{
    ...
    public bool TryAdvance(int bytes)
    {
        lock (_flowLock)
        {
            ...
            {
                throw new Http2ConnectionErrorException("The client sent more data than what was available in the flow-control window.", Http2ErrorCode.FLOW_CONTROL_ERROR, ConnectionEndReason.FlowControlWindowExceeded);
            }
