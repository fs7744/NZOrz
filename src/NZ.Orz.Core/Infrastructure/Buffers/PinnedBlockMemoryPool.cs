...
internal sealed class PinnedBlockMemoryPool : MemoryPool<byte>
{
    ...
    public override IMemoryOwner<byte> Rent(int size = AnySize)
    {
        if (size > _blockSize)
        {
            throw new ArgumentOutOfRangeException(nameof(size), $"Cannot allocate more than {_blockSize} bytes in a single buffer");
        }

        ObjectDisposedException.ThrowIf(_isDisposed, nameof(_isDisposed));
